...
if (isset($_POST['submit'])) {
    ...
    // $sql = "INSERT INTO doctor_regi (f_nm, l_nm, username, password, Email, mobile, speciality, gender, state, city, message)
    //         VALUES ('$f_nm', '$l_nm', '$U_nm', '$pass', '$Email', '$phone', '$speciality', '$gender', '$state', '$city', '$message')";

    // if ($cn->query($sql) === TRUE) {
    //     echo '<script>alert("Insertion into database successful")</script>';
    // } else {
    //     echo '<script>alert("Error inserting into database: ' . $cn->error . '")</script>';
    // }
    ...
    $objPHPExcel->getProperties()->setTitle("Doctor Registration Data")
        ->setSubject("Doctor Registration")
        ->setDescription("Data of registered doctors");
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('F1', 'Speciality')
        ->setCellValue('G1', 'Gender')
        ->setCellValue('H1', 'State')
        ->setCellValue('I1', 'City')
        ->setCellValue('J1', 'Message');

    $objPHPExcel->getActiveSheet()
        ->setCellValue('A2', $f_nm)
        ->setCellValue('B2', $l_nm)
        ->setCellValue('C2', $U_nm)
        ->setCellValue('D2', $Email)
        ->setCellValue('E2', $phone)
        ->setCellValue('F2', $speciality)
        ->setCellValue('G2', $gender)
        ->setCellValue('H2', $state)
        ->setCellValue('I2', $city)
        ->setCellValue('J2', $message);

    // Save the Excel file
    $writer = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
    $writer->save('doctor_registration.csv');

    echo '<script>alert("Insertion into Excel file successful")</script>';
}
